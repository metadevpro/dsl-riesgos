...
function resolveNodeProbability(node: NodeInfo, probabilityKey: string, maxProbability: number): number {
    ...
    if (nodeType && typeof nodeType === 'object') {
        const nodeTypeId = typeof (nodeType as Record<string, unknown>)['id'] === 'string'
            ? String((nodeType as Record<string, unknown>)['id'])
            : '';
...
function walkTheoreticalProbabilities(
    ...
    const nodeProbabilityFactor = currentId === startNodeId
        ? 1
        : resolveNodeProbability(currentNode, probabilityKey, maxProbability);
