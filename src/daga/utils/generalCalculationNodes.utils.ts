...
export function extractConnectionEndpoints(connection: unknown): ConnectionEndpoints {
    ...
    const startCandidate =
        conn['startNode'] ?? conn['sourceNode'] ?? conn['fromNode'] ?? conn['from'] ?? conn['source'] ?? conn['start'];
    const endCandidate =
        conn['endNode'] ?? conn['targetNode'] ?? conn['toNode'] ?? conn['to'] ?? conn['target'] ?? conn['end'];
...
export function getNextNodeFromConnection(
