...
export function getParentCombinations(parentIds: string[]): ParentStateCombi[][] {
    ...
    return STATES.flatMap(state =>
        restCombis.map(rc => [{ parentId: first, state }, ...rc])
    );
}
...
function enumerateWorlds(
    ...
    for (const state of ['si', 'no'] as ('si' | 'no')[]) {
        ...
        const subWorlds = enumerateWorlds(graph, order, evidence, idx + 1, { ...currentWorld, [nodeId]: state });
...
export function recalcAllMarginals(graph: BayesGraph): BayesGraph {
    ...
    for (const [nodeId, node] of graph) {
        if (node.evidence) {
            ...
            const weightSi = worlds.filter(w => w.states[nodeId] === 'si').reduce((s, w) => s + w.weight, 0);
            node.marginals = {
