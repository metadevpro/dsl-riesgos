...
export function generarDatosSinteticos(graph: BayesGraph, nMuestras = 1000, nodosOcultar: string[] = []): string {
  ...
  const edgesComment = edgePairs.length > 0 ? `# edges: ${edgePairs.join('; ')}` : '# edges:';

  return [edgesComment, headers.join(','), ...filas.map((f) => headers.map((h) => f[h] ?? '').join(','))].join('\n');
}
