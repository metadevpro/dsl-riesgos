...
function calculatePathProbability(nodePath: NodeInfo[], probabilityKey: string, maxProbability: number): number {
  if (nodePath.length === 0) return 0;

  const innerPath = nodePath.slice(1, nodePath.length - 1);
  if (innerPath.length === 0) return 1;

  return clampProbability(innerPath.reduce((acc, node) => acc * resolveNodeProbability(node, probabilityKey, maxProbability), 1));
}
